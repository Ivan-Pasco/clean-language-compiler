...
- A foundation round-trip carries **only what foundation's documents prescribe as foundation's scope**: cross-component contracts, spec text, spec gaps/silences, ADRs and the *numbers or shape* of their reference implementations. Repo-local maintenance (dependency/action bumps, lint fixes) and the owner's operational decisions (billing, repo visibility) never go in a foundation brief — even when they touch a file labeled "reference implementation".
